...
export async function broadcastState() {
  ...
  const [teamsRows, questionsRows, settingsRows] = await Promise.all([
    sql`SELECT id, team_name, total_score, logged_in FROM teams`,
    sql`SELECT id, state_code, state_name, title, image, hint, max_score, current_score, solved, solved_by FROM questions`,
